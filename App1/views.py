...
from .form import ProfileUpdate,CommentForm
from django.contrib.auth.decorators import login_required
from django.template.loader import render_to_string
from .models import user_profile,contactus,course,lecture,comment,Quiz
from .token import account_activation_token
from allcourses.models import course_details,enroll
from django.db.models import Q


def home(request):
    if request.session.has_key('is_logged'):
        return redirect('l_de')
...
def phome(request):
    if request.session.has_key('is_logged'):
        allcourse=course.objects.all()
        return render(request,"phome.html",{"allcourse":allcourse})
...
def contact(request):
    if request.method == 'POST':
        ...
        feedback.save()
        email_subject = 'Feedback submit Successfully'
...
def single(request,slug):
    if request.method == 'POST':
        ...
        comments = comment(user=request.user, course=course, discription=discription)
        comments.save()
        return redirect('home')
        ...
        comments=comment.objects.all().filter(course_id=obj)
        obj.save()
        return render(request, "course-details.html", {'obj': obj,"courses":courses,"comments":comments})


@login_required()
def main_course_single(request,title):
    m = course_details.objects.get(title=title)
    o= course_details.objects.filter(title=course_details.course).all()
    return render(request, "profile-course-details-single.html", {"m": m,"o":o})

@login_required()
def t_course(request):
    x=course_details.objects.all()
    return render(request,"test.html",{"x":x})
@login_required()
def t_single(request,slug):
    enrolled_student = request.user
    b=course_details.objects.filter(slug=slug).all()
    x= enroll.objects.filter(enrolled_student=enrolled_student)
    x=course_details.objects.filter()
    return render(request, "profile-course-details.html",{"b":b,"x":x})
...
def compiler_javascript(request):
